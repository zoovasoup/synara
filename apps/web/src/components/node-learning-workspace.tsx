...
import { LessonSurface, type ChatMessage, type CourseNode, type LessonContent } from '@/components/course-workspace-sections'
...
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle } from '@gemastik/ui/components/sheet'
...
export function NodeLearningWorkspace({ courseId, nodeId }: { courseId: string; nodeId: string }) {
  const trpc = useTRPC()
  ...
  const selectedNode = courseQuery.data?.nodes.find((node) => node.id === nodeId) ?? null
  ...
  const nodeIsAccessible = Boolean(selectedNode && selectedNode.progressionState !== 'locked')
  ...
  const tutorSessionQuery = useQuery({
    ...trpc.learning.getTutorSession.queryOptions(nodeIsAccessible ? { nodeId } : skipToken),
    enabled: nodeIsAccessible,
  }) as UseQueryResult<ChatMessage[], Error>
  const socraticSessionQuery = useQuery({
    ...trpc.validation.getSocraticSession.queryOptions(nodeIsAccessible ? { nodeId } : skipToken),
    enabled: nodeIsAccessible,
  ...
  const { clearAttempt, completeAttempt, getSnapshot, recordBacktrack } = useActiveStudyAttempt({
    nodeId: selectedNode?.progressionState === 'current' ? selectedNode.id : null,
  ...
  }, [clearAttempt, courseId, recalibration, refreshCourseData, roadmapHref, router])
  ...
  const handleSendTutorMessage = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await tutorChat.mutateAsync({ roadmapId: courseId, nodeId, message })
      queryClient.setQueryData(trpc.learning.getTutorSession.queryKey({ nodeId }), result.chatHistory)
    } catch {
  ...
  const handleSendValidationMessage = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      completeAttempt()
      setMentalEffort(5)
      await queryClient.invalidateQueries({ queryKey: trpc.validation.getSocraticSession.queryKey({ nodeId }) })
      ...
      if (navigationIntent === 'roadmap_after_recalibration') {
        toast.message("This step is taking more effort than expected. We're adjusting your learning path.")
      ...
      if (result.competency_score >= 80) {
        ...
        setSupportMode(null)
        toast.success('Step completed', { description: 'Return to the roadmap to see what is available next.' })
      } else {
  ...
  if (courseQuery.isPending || lessonContentQuery.isPending) {
    return (
      <div className='flex min-h-0 flex-1 flex-col gap-4 overflow-hidden px-4 py-4 lg:px-6'>
        <p className='text-sm font-medium' role='status'>Preparing this lesson…</p>
        <Skeleton className='mx-auto min-h-72 w-full max-w-5xl flex-1' />
      </div>
    )
  }
    ...
    return (
      <div className='flex min-h-0 flex-1 overflow-y-auto px-4 py-5 lg:px-8'>
        <div className='mx-auto flex w-full max-w-3xl flex-col gap-4'>
          <Link href={roadmapHref} className='inline-flex w-fit items-center gap-2 text-sm text-muted-foreground hover:text-foreground'>
          ...
          <Card className='border-destructive/40'>
            <CardHeader>
              ...
              <CardDescription>It may be locked, missing, or outside this course. Return to the roadmap to choose an accessible step.</CardDescription>
            </CardHeader>
          </Card>
        </div>
      </div>
    ...
    <div className='flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto px-3 py-3 sm:px-5 lg:overflow-hidden lg:px-6'>
      ...
      {completionConfirmed ? (
        <div className='mx-auto flex w-full max-w-5xl flex-col gap-3 rounded-md bg-primary/8 px-4 py-3 sm:flex-row sm:items-center sm:justify-between' role='status' aria-live='polite'>
          ...
          <Link href={roadmapHref} className={cn(buttonVariants({ variant: 'outline' }), 'w-full sm:w-auto')}>
            Back to roadmap
          </Link>
        </div>
      ) : null}
        ...
        <div className='mx-auto flex w-full max-w-5xl items-start justify-between gap-3 rounded-md bg-muted/50 px-4 py-3' role={adaptiveState === 'error' ? 'alert' : 'status'} aria-live='polite'>
          <div className='flex items-start gap-3'>
            ...
            <div>
              ...
              <p className='mt-1 text-xs text-muted-foreground'>{adaptiveState === 'pending' ? 'Mastery controls are paused while a more guided route is prepared.' : 'Try the adjustment again when you are ready.'}</p>
            </div>
          </div>
          {adaptiveState === 'error' ? (
