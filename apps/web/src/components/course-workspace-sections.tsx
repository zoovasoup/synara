...
function getResourceTypeLabel(
	type: LessonContent["resources"][number]["sourceType"],
) {
...
function getResourceLevelLabel(
	level: LessonContent["resources"][number]["level"],
) {
...
export function LessonSurface({
			...
			<section className="flex min-h-72 items-center justify-center p-8 text-center lg:h-full">
				<div className="flex max-w-sm flex-col items-center gap-2">
					...
					<p className="text-sm leading-6 text-muted-foreground">
						This draft course does not have roadmap steps. Return after its
	...
	return (
		...
		>
			<header className="shrink-0 pb-5 pt-2 md:pb-7">
				<div className="flex w-full  flex-col gap-3">
					...
					<div className="flex flex-col gap-2">
						<h1
							id="lesson-heading"
							className="text-2xl font-semibold leading-tight tracking-tight text-balance sm:text-3xl"
						>
							{selectedNode.title}
						</h1>
						<p className="max-w-2xl text-[15px] leading-7 text-foreground/78">
							{selectedNode.successCriteria[0] ??
								"Build enough understanding to explain and apply this step."}
						</p>
					</div>
					<p className="text-xs text-muted-foreground">
						{getDifficultyLabel(selectedNode.difficultyLevel)} · ~
						{selectedNode.estimatedTime} min · {selectedNode.contentType}
					</p>
				</div>
			</header>

			<div>
				<div className="flex w-full  flex-col gap-12 pb-16 pt-4 md:pt-6">
					{isPending ? (
						<div
						...
						<div role="alert" className="rounded-md bg-destructive/10 p-4">
							...
							<p className="mt-1 text-sm leading-6 text-muted-foreground">
								{errorMessage} Try opening the step again.
							</p>
						</div>
					) : lessonContent ? (
						<>
							...
							>
								...
								<ul className="flex flex-col gap-3">
									{lessonContent.concepts.map((concept) => (
										<li
											key={concept}
											className="grid grid-cols-[0.75rem_minmax(0,1fr)] gap-3 text-[15px] leading-7 text-foreground/88"
										>
											<span
												className="mt-3 size-1.5 rounded-full bg-primary"
												aria-hidden="true"
											/>
							...
							>
								...
								<ol className="flex flex-col gap-6">
									{lessonContent.steps.map((step, index) => (
										...
										>
											<span
												className="pt-1 font-mono text-xs font-medium tabular-nums text-primary"
												aria-hidden="true"
											>
												{String(index + 1).padStart(2, "0")}
											</span>
											<p className="break-words text-[15px] leading-7 text-foreground/88">
												{step}
											</p>
										</li>
									))}
							...
							>
								...
								<ul className="flex flex-col gap-3 rounded-lg border-l-2 border-primary/35 bg-muted/45 px-5 py-4">
									{lessonContent.exercises.map((exercise) => (
										<li
											key={exercise}
											className="flex gap-3 text-[15px] leading-7 text-foreground/82"
										>
											<BookOpenIcon
							...
							>
								<div className="flex flex-col gap-1">
									...
									<p className="text-sm text-muted-foreground">
										Selected from Synara’s verified catalog.
									</p>
								</div>
											...
											>
												<div className="min-w-0">
													...
													<p className="mt-1 text-xs text-muted-foreground">
														{resource.provider} ·{" "}
														{getResourceTypeLabel(resource.sourceType)} ·{" "}
														{getResourceLevelLabel(resource.level)}
													</p>
												</div>
								...
								{selectedNode.isCompleted ? (
									<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
										<div className="flex min-w-0 flex-col gap-1">
											<h2
												id="lesson-next-action-heading"
												className="text-lg font-semibold tracking-tight"
											>
												Completed
											</h2>
											<p className="text-sm leading-6 text-muted-foreground">
												Review this lesson any time, or ask the Tutor for
												another explanation.
											</p>
										</div>
										<Button
									...
									<div className="flex flex-col gap-6 rounded-lg bg-primary/8 px-5 py-6 sm:px-6">
										<div className="flex max-w-xl flex-col gap-1.5">
											...
											<p className="text-sm leading-6 text-muted-foreground">
												Explain and apply the ideas above when you are ready.
											</p>
										</div>
